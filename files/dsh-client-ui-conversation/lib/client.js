	...
	factory: (require) => {
		...
		* (running/removed/promptError) are self-selected via useSession. */
		...
		function InputBar({ useSession, useInput, inputActions, keyboard, addImages, removeImage, draftImages, resolveSubmitMode, toggleCommandMenu, stop, command, t, renderSlot, useNotices, useLexicon, useMenuLauncher, useProjection, sessionId, variant, disabled: inert = false, blocked, workspacePickerOpen = false, onRequestWorkspace, placeholder, accessory, overlay, leftItems, rightItems, footer }) {
			...
			const [textFiles, setTextFiles] = (0, react.useState)([]);
			const textFileInputRef = (0, react.useRef)(null);
			...
			const onKeyDown = (e) => {
				...
				if (textFiles.length > 0) {
					keyboard.setDraft(keyboard.snapshot.draft + textFiles.map((f) => `\n\n===== 附件文件：${f.name} =====\n${f.text}`).join("\n"));
					setTextFiles([]);
				}
			...
			const onPaste = (e) => {
				...
				if (files.length > 0) intakeDroppedFiles(files);
			...
			/** Text-file attachment budget (injected verbatim into the prompt text). */
			const TEXT_FILE_MAX_BYTES = 512 * 1024;
			/** Extension allowlist for files whose MIME is not text/* (common code/config/doc types). */
			const TEXT_FILE_EXTENSIONS = /* @__PURE__ */ new Set(["txt", "md", "markdown", "csv", "tsv", "json", "log", "yaml", "yml", "toml", "xml", "ini", "cfg", "env", "properties", "py", "js", "ts", "jsx", "tsx", "html", "css", "sh", "ps1", "sql", "r"]);
			/** Compact byte size label for file chips. */
			const fileSizeText = (size) => size > 1048576 ? `${(size / 1048576).toFixed(1)} MB` : `${Math.max(1, Math.round(size / 1024))} KB`;
			/**
			* Intake text-file attachments: MIME/extension allowlist plus a 512 KiB
			* per-file budget; every accepted file is read as UTF-8 text and cached
			* locally so submission can inject it verbatim into the prompt text.
			* @param files - browser files to consider.
			*/
			const addTextFiles = (0, react.useCallback)((files) => {
				if (files.length === 0) return;
				for (const file of files) {
					if (file.type.startsWith("image/")) continue;
					const ext = (file.name.split(".").pop() ?? "").toLowerCase();
					const isText = file.type.startsWith("text/") || TEXT_FILE_EXTENSIONS.has(ext);
					if (!isText) {
						showToast(t("file.unsupported", { name: file.name }));
						continue;
					}
					if (file.size > TEXT_FILE_MAX_BYTES) {
						showToast(t("file.tooLarge", { name: file.name, size: fileSizeText(TEXT_FILE_MAX_BYTES) }));
						continue;
					}
					const id = `tf-${Math.random().toString(36).slice(2, 10)}`;
					const reader = new FileReader();
					reader.onload = () => {
						setTextFiles((cur) => cur.some((x) => x.id === id) ? cur : [...cur, {
							id,
							name: file.name,
							size: file.size,
					...
					reader.onerror = () => {
						showToast(t("file.readFailed", { name: file.name }));
					};
					reader.readAsText(file);
				}
			}, [showToast, t]);
			/** Split dropped/pasted files: images keep the image rail, everything else goes to text files. */
			const intakeDroppedFiles = (0, react.useCallback)((files) => {
				const images = [];
				const texts = [];
				for (const file of files) {
				...
				if (texts.length > 0) addTextFiles(texts);
			}, [intakeImages, addTextFiles]);
			...
			(0, react.useEffect)(() => {
				...
				const onDrop = (event) => {
					...
					intakeDroppedFiles([...event.dataTransfer?.files ?? []]);
				};
			...
			const onPrimary = () => {
				...
				if (inputActions === void 0 || keyboard === void 0) return;
				...
				children: [
					...
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							textFiles.length > 0 && (0, react_jsx_runtime.jsx)("div", {
								className: InputBar_module_css_default.attachments,
								children: textFiles.map((f) => (0, react_jsx_runtime.jsxs)("span", {
									style: {
										display: "inline-flex",
										alignItems: "center",
										gap: 6,
									...
									"aria-label": t("file.pending"),
									children: [f.name, `（${fileSizeText(f.size)}）`, (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										style: {
											background: "none",
											border: "none",
										...
										onClick: () => {
											setTextFiles((cur) => cur.filter((x) => x.id !== f.id));
										},
					...
					(0, react_jsx_runtime.jsx)("input", {
						...
						type: "file",
						multiple: true,
						accept: ".txt,.md,.markdown,.csv,.tsv,.json,.log,.yaml,.yml,.toml,.xml,.ini,.cfg,.env,.properties,.py,.js,.ts,.jsx,.tsx,.html,.css,.sh,.ps1,.sql,.r",
						style: { display: "none" },
						onChange: (e) => {
		...
		*/
		...
		const zh = {
			...
			"input.stop": "停止生成",
			"input.send": "发送消息",
			"input.attachFile": "添加文本文件",
			"file.unsupported": "不支持的文件类型：{name}（仅支持文本类文件）",
			"file.tooLarge": "文件过大：{name}（单个文件不能超过 {size}）",
			"file.readFailed": "读取文件失败：{name}",
		...
		const en = {
			...
			"input.attachFile": "Attach text file",
			"file.unsupported": "Unsupported file type: {name} (text files only)",
			"file.tooLarge": "File too large: {name} (max {size} per file)",
			"file.readFailed": "Failed to read file: {name}",
